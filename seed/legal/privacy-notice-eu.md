...
- **Breach response**: If the user also needs breach notification procedures, hand off to a dedicated breach-notification resource — this skill stops at the privacy notice
...
- **It is not legal advice and does not replace counsel.** It assists in drafting a privacy notice; the output must be reviewed by qualified data-protection counsel or a DPO before publication. It does not create an attorney–client relationship.
- **It does not invent legal substance or citations.** Article references, supervisory-authority details, and retention citations come from the jurisdiction reference files. Where a contested point is not supported by the references, the skill flags it rather than fabricating a citation.
- **It does not produce other compliance artefacts.** It does not draft the Art. 28 DPA, the DPIA, or the breach-notification procedure — it flags when those are needed and hands off. The privacy notice deliberately excludes DPA terms, sub-processor chains, and Art. 32 technical measures.
